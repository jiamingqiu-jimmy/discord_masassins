...
def damage_check_team(cur, attacking_player_name, attacking_player_team, defending_player_name, defending_player_team):
    ...
    if sql.find_team_item(attacking_player_team, settings.item_name_x_attack) != 0:
        ...
        x_attack_string = "Attacking Team X-Attack: {} bonus damage".format(settings.x_attack_damage_bonus)
    ...
    if sql.find_team_item(defending_player_team, settings.item_name_x_defense) != 0:
        ...
        x_defense_string = "Defending Team X-Defense: {} damage negation".format(settings.x_defense_damage_negation)
    ...
    if sql.find_player_item(attacking_player_name, settings.item_name_shell_bell) != 0:
        ...
        shell_bell_string = "Attacking Player Shell Bell: {} has been healed to {}".format(settings.shell_bell_hit_healing, attacking_player_name)
...
def reward_check_player(cur, defending_player_death, attacking_player_name, attacking_player_team, defending_player_name, defending_player_team):
    ...
    if defending_player_death:
        ...
        bonus_gold_string = "Bonus gold reward: {} gold".format(settings.bonus_gold_reward)
        bonus_experience_string = "Bonus experience reward: {} experience".format(settings.bonus_experience_reward)
    ...
    if sql.find_player_item(cur, attacking_player_name, settings.item_name_amulet_coin) != 0:
        ...
        amulet_coin_gold_bonus_string = "Bonus amulet coin reward: {} gold".format(settings.amulet_coin_gold_bonus)
    ...
    if sql.find_player_item(cur, attacking_player_name, settings.item_name_expshare) != 0:
        ...
        exp_share_bonus_string = "Bonus exp share reward : {} experience".format(settings.item_name_expshare)
